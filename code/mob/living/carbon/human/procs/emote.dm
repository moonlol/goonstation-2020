	...
	//april fools end
			...
			if ("dab") //I'm honestly not sure how I'm ever going to code anything lower than this - Readster 23/04/19
				if (src.emote_check(voluntary))
					...
					if(H && (!H.limbs.l_arm || !H.limbs.r_arm))
					...
					else if ((src.mind && src.mind.assigned_role in list("Clown", "Staff Assistant", "Captain")) || (src.reagents && src.reagents.has_reagent("THC")) || (istraitor(H)) || (isnukeop(H)) || (usr.ckey == "readster") || (it_is_ass_day == 1) || !istype(src.slot_head, /obj/item/clothing/head/bighat/syndicate) || !istype(src.slot_head, /obj/item/clothing/head/bighat/syndicate/biggest)) //only clowns and the useless know the true art of dabbing
						src.mind.karma -= 10
						...
						// Act 2: Starring Firebarrage
							...
							if(H)
								...
								H.emote("scream")
						if(!istype(src.slot_head, /obj/item/clothing/head/bighat/syndicate) && !istype(src.slot_head, /obj/item/clothing/head/bighat/syndicate/biggest) || !usr.ckey == "readster")
